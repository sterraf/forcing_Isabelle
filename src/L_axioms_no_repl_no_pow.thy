...
theory L_axioms_no_repl_no_pow imports Formula Relative_no_repl_no_pow Reflection MetaExists begin
...
apply (simp add: power_ax_def powerset_def Relative_no_repl_no_pow.subset_def, clarify)
...
theorem M_trivial_L: "M_trivial_no_repl_no_pow(L)"
  apply (rule M_trivial_no_repl_no_pow.intro)
...
interpretation L?: M_trivial_no_repl_no_pow L by (rule M_trivial_L)
...
by (simp add: subset_fm_def Relative_no_repl_no_pow.subset_def)

theorem subset_reflection:
     "REFLECTS[\<lambda>x. subset(L,f(x),g(x)),
               \<lambda>i x. subset(##Lset(i),f(x),g(x))]"
apply (simp only: Relative_no_repl_no_pow.subset_def)
apply (intro FOL_reflections)
done
...
by (simp add: image_fm_def Relative_no_repl_no_pow.image_def)
...
apply (simp only: Relative_no_repl_no_pow.image_def)
apply (intro FOL_reflections pair_reflection)
done
...
by (simp add: pre_image_fm_def Relative_no_repl_no_pow.pre_image_def)
...
apply (simp only: Relative_no_repl_no_pow.pre_image_def)
apply (intro FOL_reflections pair_reflection)
done
