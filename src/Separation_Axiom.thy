...
locale sep_rename = G_generic + 
  fixes rensep :: "i"
  assumes
  rensep_action: "arity(\<phi>) \<le> 6 #+ length(env) \<Longrightarrow> [t,p,u,P,leq,one] @ env \<in> list(M) \<Longrightarrow> \<phi>\<in>formula \<Longrightarrow> 
      sats(M, rensep`\<phi>,[t,p,u,P,leq,one] @ env) \<longleftrightarrow> sats(M, \<phi>,[P,leq,one,p,t] @ env @ [u])"
  and
...
proof -  
  ...
  note in_M = \<open>\<pi>\<in>M\<close> \<open>\<sigma>\<in>M\<close> \<open>domain(\<pi>) \<times> P \<in> M\<close>  P_in_M one_in_M leq_in_M
  {
    ...
    have Eq3: "\<theta>\<in>M \<Longrightarrow> p\<in>P \<Longrightarrow>
       sats(M,?new_form,[\<theta>,p,u]@?Pl1@[\<sigma>,\<pi>]@nenv) \<longleftrightarrow>
          (\<forall>F. M_generic(F) \<and> p \<in> F \<longrightarrow> sats(M[F], ?\<chi>, [val(F, \<theta>), val(F, \<sigma>), val(F, \<pi>)] @ map(val(F), nenv)))" 
      for \<theta> p 
    proof -
      fix p \<theta> 
      ...
      let ?env="?Pl1@[p,\<theta>,\<sigma>,\<pi>] @ nenv @ [u]"
      ...
      have "sats(M,?new_form,[\<theta>,p,u]@?Pl1@[\<sigma>,\<pi>] @ nenv) \<longleftrightarrow> sats(M,?new_form,?new_env)"
        by simp
      from Eq1' \<open>forces(?\<chi>)\<in> formula\<close> \<open>arity(forces(?\<chi>)) \<le> 7 #+ length(env)\<close> \<open>length(nenv) = length(env)\<close>
      have "... \<longleftrightarrow> sats(M,forces(?\<chi>),?env)"
        using rensep_action[of "forces(?\<chi>)" "[\<sigma>,\<pi>] @ nenv" \<theta> p u] leI by simp
      also 
      have "... \<longleftrightarrow> sats(M,forces(?\<chi>), ([P, leq, one,p,\<theta>,\<sigma>,\<pi>]@nenv)@[u])" by simp
      also 
      ...
      have "... \<longleftrightarrow> sats(M,forces(?\<chi>), [P, leq, one,p,\<theta>,\<sigma>,\<pi>]@nenv)"        
      ...
      have " ... \<longleftrightarrow> (\<forall>F. M_generic(F) \<and> p \<in> F \<longrightarrow> 
                           sats(M[F], ?\<chi>, [val(F, \<theta>), val(F, \<sigma>), val(F, \<pi>)] @ map(val(F), nenv)))"
        using  definition_of_forces 
      proof (intro iffI)
        assume a1: "sats(M, forces(?\<chi>), [P, leq, one,p,\<theta>,\<sigma>,\<pi>] @ nenv)"
        ...
        have "p \<in> P \<Longrightarrow> ?\<chi>\<in>formula \<Longrightarrow> [\<theta>,\<sigma>,\<pi>] \<in> list(M) \<Longrightarrow>
                  sats(M, forces(?\<chi>), [P, leq, one, p] @ [\<theta>,\<sigma>,\<pi>]@ nenv) \<Longrightarrow> 
              \<forall>G. M_generic(G) \<and> p \<in> G \<longrightarrow> sats(M[G], ?\<chi>, map(val(G), [\<theta>,\<sigma>,\<pi>] @ nenv))" by simp
        then
      ...
      next
        ...
        show "sats(M, forces(?\<chi>), [P, leq, one,p,\<theta>,\<sigma>,\<pi>] @ nenv)"
  ...
  have Equivalence: "u\<in> domain(\<pi>) \<times> P \<Longrightarrow> u \<in> M \<Longrightarrow> 
    ...
    for u 
    by simp
  with generic \<open>env = _\<close>
  have "(\<exists>\<theta>\<in>M. \<exists>p\<in>P. u =\<langle>\<theta>,p\<rangle> \<and> (p\<in>G \<longrightarrow> sats(M[G],?\<chi>,[val(G,\<theta>), val(G,\<sigma>), val(G,\<pi>)] @ env)))" 
   (is "(\<exists>\<theta>\<in>M. \<exists>p\<in>P. _ ( _ \<longrightarrow> sats(_,_,?vals(\<theta>))))")
    if "u \<in> domain(\<pi>) \<times> P" "u \<in> M"  "sats(M,?\<psi>,[u, P, leq, one,\<sigma>, \<pi>] @ nenv)" for u
    using that
    by force
  moreover 
  ...
  have "(\<exists>\<theta>\<in>M. \<exists>p\<in>P. u=\<langle>\<theta>,p\<rangle> \<and> (p\<in>G \<longrightarrow> val(G,\<theta>)\<in>val(G,\<pi>) \<and> sats(M[G], \<phi>, ?vals(\<theta>))))"
    if "u \<in> domain(\<pi>) \<times> P" "u \<in> M"  "sats(M,?\<psi>,[u, P, leq, one,\<sigma>, \<pi>] @ nenv)" for u
    using that \<open>\<pi>\<in>M\<close> \<open>env \<in> _\<close> by auto
  ...
  have "val(G,?m) =
            ...
            (p \<in> G \<longrightarrow> val(G, \<theta>) \<in> c \<and> sats(M[G], \<phi>, [val(G, \<theta>), w, c] @ env)) \<and> q \<in> G)}"
    using val_of_name by auto
  also 
  have "... =  {val(G,t) .. t\<in>domain(\<pi>) , \<exists>q\<in>P. 
                   val(G, t) \<in> c \<and> sats(M[G], \<phi>, [val(G, t), w, c] @ env) \<and> q \<in> G}" 
  proof -
    ...
    have "t\<in>M \<Longrightarrow>
      (\<exists>q\<in>P. (\<exists>\<theta>\<in>M. \<exists>p\<in>P. <t,q> = \<langle>\<theta>, p\<rangle> \<and> 
              (p \<in> G \<longrightarrow> val(G, \<theta>) \<in> c \<and> sats(M[G], \<phi>, [val(G, \<theta>), w, c] @ env)) \<and> q \<in> G)) 
      \<longleftrightarrow> 
      (\<exists>q\<in>P. val(G, t) \<in> c \<and> sats(M[G], \<phi>, [val(G, t), w, c] @ env) \<and> q \<in> G)" for t
      by auto
    then show ?thesis using \<open>domain(\<pi>)\<in>M\<close> by (auto simp add:transitivity)
  qed
  also 
  have "... =  {x .. x\<in>c , \<exists>q\<in>P. x \<in> c \<and> sats(M[G], \<phi>, [x, w, c] @ env) \<and> q \<in> G}"
  proof

    show "... \<subseteq> {x .. x\<in>c , \<exists>q\<in>P. x \<in> c \<and> sats(M[G], \<phi>, [x, w, c] @ env) \<and> q \<in> G}"
      by auto
  next 
    ...
    *)
    {
      fix x
      assume "x\<in>{x .. x\<in>c , \<exists>q\<in>P. x \<in> c \<and> sats(M[G], \<phi>, [x, w, c] @ env) \<and> q \<in> G}"
      then 
      have "\<exists>q\<in>P. x \<in> c \<and> sats(M[G], \<phi>, [x, w, c] @ env) \<and> q \<in> G"
        by simp
      with \<open>val(G,\<pi>) = c\<close>  
      have "\<exists>q\<in>P. \<exists>t\<in>domain(\<pi>). val(G,t) =x \<and> sats(M[G], \<phi>, [val(G,t), w, c] @ env) \<and> q \<in> G" 
        using Sep_and_Replace elem_of_val by auto
    }
    then 
    show " {x .. x\<in>c , \<exists>q\<in>P. x \<in> c \<and> sats(M[G], \<phi>, [x, w, c] @ env) \<and> q \<in> G} \<subseteq> ..."
      using SepReplace_iff by force
  qed
  also 
  have " ... = {x\<in>c. sats(M[G], \<phi>, [x, w, c] @ env)}"
    using \<open>G\<subseteq>P\<close> G_nonempty by force
  finally 
  have val_m: "val(G,?m) = {x\<in>c. sats(M[G], \<phi>, [x, w, c] @ env)}" by simp
  have "val(G,?m) \<subseteq> val(G,?n)" 
  proof
    fix x
    assume "x \<in> val(G,?m)"
    with val_m 
    have Eq4: "x \<in> {x\<in>c. sats(M[G], \<phi>, [x, w, c] @ env )}" by simp
    with \<open>val(G,\<pi>) = c\<close> 
    ...
    have Eq5: "sats(M[G], And(Member(0,2),\<phi>), [val(G,\<theta>), val(G,\<sigma>), val(G,\<pi>)] @ env)" 
      by auto
        (* Recall ?\<chi> = And(Member(0,2),\<phi>) *)
    with \<open>\<theta>\<in>M\<close> \<open>\<pi>\<in>M\<close>  \<open>\<sigma>\<in>M\<close> Eq5 \<open>M_generic(G)\<close> \<open>\<phi>\<in>formula\<close> \<open>nenv \<in> _ \<close> \<open>env = _ \<close>
    have "(\<exists>r\<in>G. sats(M,forces(?\<chi>), [P,leq,one,r,\<theta>,\<sigma>,\<pi>] @ nenv))"
      using truth_lemma  by auto
    then obtain r where      (* I can't "obtain" this directly *)
      "r\<in>G" "sats(M,forces(?\<chi>), [P,leq,one,r,\<theta>,\<sigma>,\<pi>] @ nenv)" by auto
    ...
    with \<open>\<phi>\<in>formula\<close> \<open>\<theta>\<in>M\<close> \<open>\<pi>\<in>M\<close> \<open>\<sigma>\<in>M\<close>  \<open><p,r>\<in>leq\<close> \<open>nenv \<in> _\<close>
      \<open>sats(M,forces(?\<chi>), [P,leq,one,r,\<theta>,\<sigma>,\<pi>] @ nenv)\<close> 
    have "sats(M,forces(?\<chi>), [P,leq,one,p,\<theta>,\<sigma>,\<pi>] @ nenv)"
    ...
    have Eq6: "\<exists>\<theta>'\<in>M. \<exists>p'\<in>P.  \<langle>\<theta>,p\<rangle> = <\<theta>',p'> \<and> (\<forall>F. M_generic(F) \<and> p' \<in> F \<longrightarrow> 
                 sats(M[F], ?\<chi>, [val(F, \<theta>'), val(F, \<sigma>), val(F,\<pi>)] @ map(val(F), nenv)))" by auto
