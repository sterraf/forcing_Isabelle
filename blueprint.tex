...
Since the  function $F$ will essentially have 
...
\begin{align}
  ...
  \forceisa(\quine{t_1=t_2},\PP,\leq,\1,p) \label{eq:forcing-F-equ}\\
  F(\lb\bel,s,t_1,\PP,\leq,\1,p\rb) =1 &\iff  
  \forceisa(\quine{s\in t_1},\PP,\leq,\1,q). \label{eq:forcing-F-bel}
\end{align}
...
\begin{framed}
  ...
  well-founded independently of universe where one is working. We only
