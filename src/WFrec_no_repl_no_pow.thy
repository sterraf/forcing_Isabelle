...
theory WFrec_no_repl_no_pow imports Wellorderings_no_repl_no_pow begin
...
lemma (in M_basic_no_repl_no_pow) is_recfun_separation':
...
lemma (in M_basic_no_repl_no_pow) is_recfun_equal [rule_format]: 
...
lemma (in M_basic_no_repl_no_pow) is_recfun_cut: 
    "[|is_recfun(r,a,H,f);  is_recfun(r,b,H,g);  
       wellfounded(M,r); trans(r); 
...
lemma (in M_basic_no_repl_no_pow) is_recfun_functional:
...
lemma (in M_basic_no_repl_no_pow) is_recfun_relativize:
...
lemma (in M_basic_no_repl_no_pow) is_recfun_restrict:
...
lemma (in M_basic_no_repl_no_pow) restrict_Y_lemma:
...
lemma (in M_basic_no_repl_no_pow) univalent_is_recfun:
     "[|wellfounded(M,r); trans(r); M(r)|]
...
lemma (in M_basic_no_repl_no_pow) exists_is_recfun_indstep:
...
lemma (in M_basic_no_repl_no_pow) wellfounded_exists_is_recfun:
    "[|wellfounded(M,r);  trans(r);  
...
lemma (in M_basic_no_repl_no_pow) wf_exists_is_recfun [rule_format]:
    "[|wf(r);  trans(r);  M(r);  
       strong_replacement(M, \<lambda>x z. 
         \<exists>y[M]. \<exists>g[M]. pair(M,x,y,z) & is_recfun(r,x,H,g) & y = H(x,g)); 
       \<forall>x[M]. \<forall>g[M]. function(g) \<longrightarrow> M(H(x,g)) |]   
...
lemma (in M_basic_no_repl_no_pow) is_recfun_abs:
...
lemma (in M_basic_no_repl_no_pow) is_wfrec_abs:
     "[| \<forall>x[M]. \<forall>g[M]. function(g) \<longrightarrow> M(H(x,g)); 
...
lemma (in M_basic_no_repl_no_pow) wfrec_replacement':
  "[|wfrec_replacement(M,MH,r);
     \<forall>x[M]. \<forall>g[M]. function(g) \<longrightarrow> M(H(x,g)); 
