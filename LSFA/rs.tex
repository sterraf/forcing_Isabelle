...
version of   the \emph{Principle of Dependent Choices} which,
...
Grabczewski~\cite{DBLP:journals/jar/PaulsonG96}, as part of a
study of equivalents to $\AC$ (following Rubin and Rubin
\cite{rubin1985equivalents}). Nevertheless, those are not convenient
for our purposes for the reasons we now explain. The axiom $\DC0$
corresponds essentially to our Pointed $\DC$ but without the
constraint $f(0)=a$; it is a nice exercise to show that $\DC0$ implies
Pointed $\DC$, but a formalization would have a moderate length. On
the other hand, the shortest path from $\AC$ as stated in the
corresponding Isabelle theory to $\DC0$ using the already formalized
material involves a complicated detour (130+ proof commands spanning
various files of the \isatt{ZF-AC} theory), compared to the mere 11 commands from $\AC$ to
...
The proof is mostly routine; after a few lemmas (having 26 proof
