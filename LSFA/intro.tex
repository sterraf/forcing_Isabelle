...
Assume that $M$ is a ctm of $\ZFC$. It is instructive to sketch
G\"odel's argument of the relative consistency of $\CH$: he showed
that $M$ contains a minimal submodel $L^M$ of the same ``height''
(i.e.\ having the same ordinals) that satisfies $\ZFC+\CH$. The sets
in $L^M$ are called \emph{constructible} and are in a sense
``definable.'' In fact, there is a first-order formula $L$ such that
$L^M = \{x\in M : M\models L(x)\}$. To show that
...
from \url{https://cs.famaf.unc.edu.ar/~pedro/forcing/}.
