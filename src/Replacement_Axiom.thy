...
definition renrep_fn :: "i \<Rightarrow> i" where
  "renrep_fn(n) == sum(renrep1_fn,id(n),6,8,n)"

lemma renrep_fn_type :
  assumes "n\<in>nat"
  shows "renrep_fn(n) \<in> 6#+n \<rightarrow> 8#+n"
...
lemma renrep_fn_action : 
  assumes 
    "[P,leq,o,p,\<rho>,\<tau>] \<in> list(M)" 
    "env \<in> list(M)"
    "V \<in> M" "\<alpha> \<in> M"
  shows "\<And> i . i < 6 #+ length(env) \<Longrightarrow>
    nth(i, [P,leq,o,p,\<rho>,\<tau>] @ env) = nth(renrep_fn(length(env))`i, [V,\<tau>,\<rho>,p,\<alpha>,P,leq,o] @ env)"
proof - 
  from assms
  have 2:"[V,\<tau>,\<rho>,p,\<alpha>,P,leq,o] \<in> list(M)" 
       " {P, leq, o, p, \<rho>, \<tau>} \<subseteq> M " by simp_all
  let ?env1 = "[P,leq,o,p,\<rho>,\<tau>]"
  let ?env2 = "[V,\<tau>,\<rho>,p,\<alpha>,P,leq,o]" 
  let ?n = "length(env)"
  from \<open>env\<in>list(M)\<close> 
  have "length(env)\<in>nat" by simp
  then show "nth(i, [P,leq,o,p,\<rho>,\<tau>] @ env) = nth(renrep_fn(length(env))`i, [V,\<tau>,\<rho>,p,\<alpha>,P,leq,o] @ env)"
    if "i < 6 #+ length(env)" for i
     unfolding renrep_fn_def renrep1_def
     using \<open>?n\<in>nat\<close> that
       sum_action[OF _ _ \<open>?n\<in>nat\<close> \<open>?n\<in>nat\<close> renrep_thm(2)  id_fn_type
                        \<open>?env1 \<in> list(M)\<close> \<open>?env2 \<in> list(M)\<close> \<open>env\<in>list(M)\<close> \<open>env\<in>list(M)\<close>]
                         renrep_thm(1)[of P leq o p \<rho> \<tau> M,OF 2(2)]
                         id_fn_action
     by simp
qed
...
definition 
  ...
  "renrep(\<phi>,n) = ren(\<phi>)`(6#+n)`(8#+n)`renrep_fn(n)" 
