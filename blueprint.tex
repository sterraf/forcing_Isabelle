...
%% variables. 
Informally, we call $\forces$ the \emph{forcing relation}.
...
The base case (for atomic formulas) is famously the most
complicated one. 
\begin{multline}
...
Actually, these cases depend on the theorem of well-founded recursion,
so the formula $\forceisa(\quine\phi,\PP,\leq,\1,p)$ for atomic $\phi$
must be obtained from the proof of that theorem. This means that we need
an internalized version of the theorem.
