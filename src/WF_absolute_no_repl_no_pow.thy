...
theory WF_absolute_no_repl_no_pow imports WFrec_no_repl_no_pow begin
...
locale M_trancl_no_repl_no_pow = M_basic_no_repl_no_pow +
...
lemma (in M_trancl_no_repl_no_pow) rtran_closure_mem_iff:
     "[|M(A); M(r); M(p)|]
...
lemma (in M_trancl_no_repl_no_pow) rtran_closure_rtrancl:
...
lemma (in M_trancl_no_repl_no_pow) rtrancl_closed [intro,simp]:
     "M(r) ==> M(rtrancl(r))"
...
lemma (in M_trancl_no_repl_no_pow) rtrancl_abs [simp]:
...
lemma (in M_trancl_no_repl_no_pow) trancl_closed [intro,simp]:
     "M(r) ==> M(trancl(r))"
by (simp add: trancl_def)

lemma (in M_trancl_no_repl_no_pow) trancl_abs [simp]:
     "[| M(r); M(z) |] ==> tran_closure(M,r,z) \<longleftrightarrow> z = trancl(r)"
by (simp add: tran_closure_def trancl_def)

lemma (in M_trancl_no_repl_no_pow) wellfounded_trancl_separation':
...
lemma (in M_trancl_no_repl_no_pow) wellfounded_on_trancl:
...
lemma (in M_trancl_no_repl_no_pow) wellfounded_trancl:
...
lemma (in M_trancl_no_repl_no_pow) wfrec_relativize:
  "[|wf(r); M(a); M(r);  
...
theorem (in M_trancl_no_repl_no_pow) trans_wfrec_relativize:
...
theorem (in M_trancl_no_repl_no_pow) trans_wfrec_abs:
...
lemma (in M_trancl_no_repl_no_pow) trans_eq_pair_wfrec_iff:
...
lemma (in M_trancl_no_repl_no_pow) wfrec_closed_lemma [rule_format]:
     "[|wf(r); M(r); 
...
lemma (in M_trancl_no_repl_no_pow) wfrec_replacement_iff:
...
theorem (in M_trancl_no_repl_no_pow) trans_wfrec_closed:
     "[|wf(r); trans(r); relation(r); M(r); M(a);
       wfrec_replacement(M,MH,r);  relation2(M,MH,H);
        \<forall>x[M]. \<forall>g[M]. function(g) \<longrightarrow> M(H(x,g)) |] 
      ==> M(wfrec(r,a,H))"
...
lemma (in M_trancl_no_repl_no_pow) eq_pair_wfrec_iff:
  "[|wf(r);  M(r);  M(y); 
     strong_replacement(M, \<lambda>x z. \<exists>y[M]. \<exists>g[M].
          pair(M,x,y,z) & 
          is_recfun(r^+, x, \<lambda>x f. H(x, restrict(f, r -`` {x})), g) & 
...
theorem (in M_trancl_no_repl_no_pow) wfrec_closed:
     "[|wf(r); M(r); M(a);
