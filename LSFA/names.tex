...
In our case the functional $H$ takes an additional argument for the
parameter $G$:
\[
  H(G,y,f) = \{f(x) : x \in dom(y) \y \exists p\in \PP.\,
...
In the formalization of $\mathit{ed}$ is restricted to a set,
so to have a term of type \isa{i}:
% 
\begin{isabelle}
...
\noindent We omit the proof of the following lemma used above:
...
\noindent The formalizaton of the functional $H$ is straightforward and $\val$ is defined using \isa{wfrec}:
%
\begin{isabelle}
\isacommand{definition}\isamarkupfalse%
\isanewline
