...
\isatt{replacement{\uscore}assm(M,\isasymphi)} predicate:
...
\session{ZF-Constructible} and we included for simplicity of design,
are actually “fake” instances in that they can be obtained from
Powerset. An example of this is the sole instance of the
\isatt{M{\uscore}basic} locale; we managed to eliminate it but in
doing so we had to reprove many lemmas from
\session{ZF-Constructible} in the weakened context. Our locales
