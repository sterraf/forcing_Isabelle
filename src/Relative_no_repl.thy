theory Relative_no_repl imports Main begin
...
lemma (in M_trivial_no_repl) rex_abs [simp]:
     "M(A) ==> (\<exists>x[M]. x\<in>A & P(x)) \<longleftrightarrow> (\<exists>x\<in>A. P(x))"
by (blast intro: transM)

lemma (in M_trivial_no_repl) ball_iff_equiv:
...
lemma (in M_trivial_no_repl) M_equalityI:
...
lemma (in M_trivial_no_repl) union_abs [simp]:
     "[| M(a); M(b); M(z) |] ==> union(M,a,b,z) \<longleftrightarrow> z = a \<union> b"
apply (simp add: union_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl) inter_abs [simp]:
     "[| M(a); M(b); M(z) |] ==> inter(M,a,b,z) \<longleftrightarrow> z = a \<inter> b"
apply (simp add: inter_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl) setdiff_abs [simp]:
...
lemma (in M_trivial_no_repl) Union_closed [intro,simp]:
     "M(A) ==> M(\<Union>(A))"
by (insert Union_ax, simp add: Union_ax_def)

lemma (in M_trivial_no_repl) Un_closed [intro,simp]:
     "[| M(A); M(B) |] ==> M(A \<union> B)"
by (simp only: Un_eq_Union, blast)

lemma (in M_trivial_no_repl) cons_closed [intro,simp]:
     "[| M(a); M(A) |] ==> M(cons(a,A))"
by (subst cons_eq [symmetric], blast)

lemma (in M_trivial_no_repl) cons_abs [simp]:
     "[| M(b); M(z) |] ==> is_cons(M,a,b,z) \<longleftrightarrow> z = cons(a,b)"
by (simp add: is_cons_def, blast intro: transM)

lemma (in M_trivial_no_repl) successor_abs [simp]:
     "[| M(a); M(z) |] ==> successor(M,a,z) \<longleftrightarrow> z = succ(a)"
by (simp add: successor_def, blast)

lemma (in M_trivial_no_repl) succ_in_M_iff [iff]:
...
lemma (in M_trivial_no_repl) separation_closed [intro,simp]:
...
lemma (in M_trivial_no_repl) Collect_abs [simp]:
...
lemma (in M_trivial_no_repl) strong_replacementI [rule_format]:
...
lemma (in M_trivial_no_repl) univalent_Replace_iff:
     "[| M(A); univalent(M,A,P);
...
lemma (in M_trivial_no_repl) strong_replacement_closed [intro,simp]:
     "[| strong_replacement(M,P); M(A); univalent(M,A,P);
...
lemma (in M_trivial_no_repl) Replace_abs:
...
(*The first premise can't simply be assumed as a schema.
  ...
  even for f \<in> M -> M.
*)
lemma (in M_trivial_no_repl) RepFun_closed:
...
lemma (in M_trivial_no_repl) RepFun_closed2:
...
lemma (in M_trivial_no_repl) lam_closed:
...
lemma (in M_trivial_no_repl) lam_closed2:
...
lemma (in M_trivial_no_repl) lambda_abs2:
...
lemma (in M_trivial_no_repl) image_abs [simp]:
...
lemma (in M_trivial_no_repl) powerset_Pow:
     "powerset(M, x, Pow(x))"
by (simp add: powerset_def)

text\<open>But we can't prove that the powerset in \<open>M\<close> includes the
      real powerset.\<close>
lemma (in M_trivial_no_repl) powerset_imp_subset_Pow:
...
lemma (in M_trivial_no_repl) nat_into_M [intro]:
     "n \<in> nat ==> M(n)"
by (induct n rule: nat_induct, simp_all)

lemma (in M_trivial_no_repl) nat_case_closed [intro,simp]:
...
lemma (in M_trivial_no_repl) lt_closed:
     "[| j<i; M(i) |] ==> M(j)"
by (blast dest: ltD intro: transM)

lemma (in M_trivial_no_repl) transitive_set_abs [simp]:
     "M(a) ==> transitive_set(M,a) \<longleftrightarrow> Transset(a)"
by (simp add: transitive_set_def Transset_def)

lemma (in M_trivial_no_repl) ordinal_abs [simp]:
     "M(a) ==> ordinal(M,a) \<longleftrightarrow> Ord(a)"
by (simp add: ordinal_def Ord_def)

lemma (in M_trivial_no_repl) limit_ordinal_abs [simp]:
...
lemma (in M_trivial_no_repl) successor_ordinal_abs [simp]:
...
lemma (in M_trivial_no_repl) finite_ordinal_abs [simp]:
...
lemma (in M_trivial_no_repl) Inl_in_M_iff [iff]:
     "M(Inl(a)) \<longleftrightarrow> M(a)"
by (simp add: Inl_def)

lemma (in M_trivial_no_repl) Inl_abs [simp]:
     "M(Z) ==> is_Inl(M,a,Z) \<longleftrightarrow> (Z = Inl(a))"
by (simp add: is_Inl_def Inl_def)

lemma (in M_trivial_no_repl) Inr_in_M_iff [iff]:
     "M(Inr(a)) \<longleftrightarrow> M(a)"
by (simp add: Inr_def)

lemma (in M_trivial_no_repl) Inr_abs [simp]:
...
lemma (in M_basic_no_repl) domain_abs [simp]:
...
lemma (in M_basic_no_repl) range_abs [simp]:
...
lemma (in M_basic_no_repl) relation_abs [simp]:
...
lemma (in M_basic_no_repl) function_abs [simp]:
...
lemma (in M_basic_no_repl) typed_function_abs [simp]:
...
lemma (in M_basic_no_repl) restriction_abs [simp]:
     "[| M(f); M(A); M(z) |]
...
lemma (in M_basic_no_repl) M_restrict_iff:
...
lemma (in M_basic_no_repl) Inter_abs [simp]:
...
lemma (in M_trivial_no_repl) Collect_rall_eq:
...
lemma (in M_basic_no_repl) is_funspace_abs [simp]:
...
lemma (in M_trivial_no_repl) Nil_in_M [intro,simp]: "M(Nil)"
by (simp add: Nil_def)

lemma (in M_trivial_no_repl) Nil_abs [simp]: "M(Z) ==> is_Nil(M,Z) \<longleftrightarrow> (Z = Nil)"
by (simp add: is_Nil_def Nil_def)

lemma (in M_trivial_no_repl) Cons_in_M_iff [iff]: "M(Cons(a,l)) \<longleftrightarrow> M(a) & M(l)"
...
lemma (in M_trivial_no_repl) is_hd_Nil: "is_hd(M,[],Z) \<longleftrightarrow> empty(M,Z)"
by (simp add: is_hd_def)

lemma (in M_trivial_no_repl) is_hd_Cons:
     "[|M(a); M(l)|] ==> is_hd(M,Cons(a,l),Z) \<longleftrightarrow> Z = a"
by (force simp add: is_hd_def)

lemma (in M_trivial_no_repl) hd_abs [simp]:
...
lemma (in M_trivial_no_repl) is_tl_Nil: "is_tl(M,[],Z) \<longleftrightarrow> Z = []"
by (simp add: is_tl_def)

lemma (in M_trivial_no_repl) is_tl_Cons:
     "[|M(a); M(l)|] ==> is_tl(M,Cons(a,l),Z) \<longleftrightarrow> Z = l"
by (force simp add: is_tl_def)

lemma (in M_trivial_no_repl) tl_abs [simp]:
...
lemma (in M_trivial_no_repl) relation1_tl: "relation1(M, is_tl(M), tl')"
by (simp add: relation1_def)
