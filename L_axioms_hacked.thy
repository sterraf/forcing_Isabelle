...
theory L_axioms_hacked imports Formula Relative_hacked Reflection MetaExists begin
...
apply (simp add: power_ax_def powerset_def Relative_hacked.subset_def, clarify)
...
theorem M_trivial_L: "M_trivial_no_repl(L)"
...
by (simp add: subset_fm_def Relative_hacked.subset_def)

theorem subset_reflection:
     "REFLECTS[\<lambda>x. subset(L,f(x),g(x)),
               \<lambda>i x. subset(##Lset(i),f(x),g(x))]"
apply (simp only: Relative_hacked.subset_def)
apply (intro FOL_reflections)
done
...
by (simp add: image_fm_def Relative_hacked.image_def)
...
apply (simp only: Relative_hacked.image_def)
apply (intro FOL_reflections pair_reflection)
done
...
by (simp add: pre_image_fm_def Relative_hacked.pre_image_def)
...
apply (simp only: Relative_hacked.pre_image_def)
apply (intro FOL_reflections pair_reflection)
done
