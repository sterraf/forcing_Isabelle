theory Relative_no_repl_no_pow imports Main begin
...
locale M_trivial_no_repl_no_pow =
  fixes M
...
lemma (in M_trivial_no_repl_no_pow) nonempty [simp]: "M(0)"
by (blast intro: transM)

lemma (in M_trivial_no_repl_no_pow) rall_abs [simp]:
     "M(A) ==> (\<forall>x[M]. x\<in>A \<longrightarrow> P(x)) \<longleftrightarrow> (\<forall>x\<in>A. P(x))"
by (blast intro: transM)

lemma (in M_trivial_no_repl_no_pow) rex_abs [simp]:
     "M(A) ==> (\<exists>x[M]. x\<in>A & P(x)) \<longleftrightarrow> (\<exists>x\<in>A. P(x))"
by (blast intro: transM)

lemma (in M_trivial_no_repl_no_pow) ball_iff_equiv:
...
lemma (in M_trivial_no_repl_no_pow) M_equalityI:
...
lemma (in M_trivial_no_repl_no_pow) empty_abs [simp]:
     "M(z) ==> empty(M,z) \<longleftrightarrow> z=0"
apply (simp add: empty_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) subset_abs [simp]:
     "M(A) ==> subset(M,A,B) \<longleftrightarrow> A \<subseteq> B"
apply (simp add: subset_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) upair_abs [simp]:
     "M(z) ==> upair(M,a,b,z) \<longleftrightarrow> z={a,b}"
apply (simp add: upair_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) upair_in_M_iff [iff]:
     "M({a,b}) \<longleftrightarrow> M(a) & M(b)"
apply (insert upair_ax, simp add: upair_ax_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) singleton_in_M_iff [iff]:
     "M({a}) \<longleftrightarrow> M(a)"
by (insert upair_in_M_iff [of a a], simp)

lemma (in M_trivial_no_repl_no_pow) pair_abs [simp]:
     "M(z) ==> pair(M,a,b,z) \<longleftrightarrow> z=<a,b>"
apply (simp add: pair_def ZF.Pair_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) pair_in_M_iff [iff]:
     "M(<a,b>) \<longleftrightarrow> M(a) & M(b)"
by (simp add: ZF.Pair_def)

lemma (in M_trivial_no_repl_no_pow) pair_components_in_M:
     "[| <x,y> \<in> A; M(A) |] ==> M(x) & M(y)"
apply (simp add: Pair_def)
apply (blast dest: transM)
done

lemma (in M_trivial_no_repl_no_pow) cartprod_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) union_abs [simp]:
     "[| M(a); M(b); M(z) |] ==> union(M,a,b,z) \<longleftrightarrow> z = a \<union> b"
apply (simp add: union_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) inter_abs [simp]:
     "[| M(a); M(b); M(z) |] ==> inter(M,a,b,z) \<longleftrightarrow> z = a \<inter> b"
apply (simp add: inter_def)
apply (blast intro: transM)
done

lemma (in M_trivial_no_repl_no_pow) setdiff_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) Union_closed [intro,simp]:
     "M(A) ==> M(\<Union>(A))"
by (insert Union_ax, simp add: Union_ax_def)

lemma (in M_trivial_no_repl_no_pow) Un_closed [intro,simp]:
     "[| M(A); M(B) |] ==> M(A \<union> B)"
by (simp only: Un_eq_Union, blast)

lemma (in M_trivial_no_repl_no_pow) cons_closed [intro,simp]:
     "[| M(a); M(A) |] ==> M(cons(a,A))"
by (subst cons_eq [symmetric], blast)

lemma (in M_trivial_no_repl_no_pow) cons_abs [simp]:
     "[| M(b); M(z) |] ==> is_cons(M,a,b,z) \<longleftrightarrow> z = cons(a,b)"
by (simp add: is_cons_def, blast intro: transM)

lemma (in M_trivial_no_repl_no_pow) successor_abs [simp]:
     "[| M(a); M(z) |] ==> successor(M,a,z) \<longleftrightarrow> z = succ(a)"
by (simp add: successor_def, blast)

lemma (in M_trivial_no_repl_no_pow) succ_in_M_iff [iff]:
...
lemma (in M_trivial_no_repl_no_pow) separation_closed [intro,simp]:
...
lemma (in M_trivial_no_repl_no_pow) Collect_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) strong_replacementI [rule_format]:
...
lemma (in M_trivial_no_repl_no_pow) univalent_Replace_iff:
     "[| M(A); univalent(M,A,P);
...
lemma (in M_trivial_no_repl_no_pow) strong_replacement_closed [intro,simp]:
     "[| strong_replacement(M,P); M(A); univalent(M,A,P);
...
lemma (in M_trivial_no_repl_no_pow) Replace_abs:
...
(*The first premise can't simply be assumed as a schema.
  ...
  even for f \<in> M -> M.
*)
lemma (in M_trivial_no_repl_no_pow) RepFun_closed:
...
lemma (in M_trivial_no_repl_no_pow) RepFun_closed2:
...
lemma (in M_trivial_no_repl_no_pow) lam_closed:
...
lemma (in M_trivial_no_repl_no_pow) lam_closed2:
...
lemma (in M_trivial_no_repl_no_pow) lambda_abs2:
...
lemma (in M_trivial_no_repl_no_pow) image_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) powerset_Pow:
     "powerset(M, x, Pow(x))"
by (simp add: powerset_def)

text\<open>But we can't prove that the powerset in \<open>M\<close> includes the
      real powerset.\<close>
lemma (in M_trivial_no_repl_no_pow) powerset_imp_subset_Pow:
...
lemma (in M_trivial_no_repl_no_pow) nat_into_M [intro]:
     "n \<in> nat ==> M(n)"
by (induct n rule: nat_induct, simp_all)

lemma (in M_trivial_no_repl_no_pow) nat_case_closed [intro,simp]:
...
lemma (in M_trivial_no_repl_no_pow) quasinat_abs [simp]:
     "M(z) ==> is_quasinat(M,z) \<longleftrightarrow> quasinat(z)"
by (auto simp add: is_quasinat_def quasinat_def)

lemma (in M_trivial_no_repl_no_pow) nat_case_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) lt_closed:
     "[| j<i; M(i) |] ==> M(j)"
by (blast dest: ltD intro: transM)

lemma (in M_trivial_no_repl_no_pow) transitive_set_abs [simp]:
     "M(a) ==> transitive_set(M,a) \<longleftrightarrow> Transset(a)"
by (simp add: transitive_set_def Transset_def)

lemma (in M_trivial_no_repl_no_pow) ordinal_abs [simp]:
     "M(a) ==> ordinal(M,a) \<longleftrightarrow> Ord(a)"
by (simp add: ordinal_def Ord_def)

lemma (in M_trivial_no_repl_no_pow) limit_ordinal_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) successor_ordinal_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) finite_ordinal_abs [simp]:
...
(*Could not get this to work.  The \<lambda>x\<in>nat is essential because everything
  ...
  lemma (in M_trivial_no_repl_no_pow) [simp]:
       "natnumber(M,0,x) == x=0"
*)

subsection\<open>Some instances of separation and strong replacement\<close>

locale M_basic_no_repl_no_pow = M_trivial_no_repl_no_pow +
...
lemma (in M_basic_no_repl_no_pow) cartprod_iff_lemma:
...
lemma (in M_trivial_no_repl_no_pow) Inl_in_M_iff [iff]:
     "M(Inl(a)) \<longleftrightarrow> M(a)"
by (simp add: Inl_def)

lemma (in M_trivial_no_repl_no_pow) Inl_abs [simp]:
     "M(Z) ==> is_Inl(M,a,Z) \<longleftrightarrow> (Z = Inl(a))"
by (simp add: is_Inl_def Inl_def)

lemma (in M_trivial_no_repl_no_pow) Inr_in_M_iff [iff]:
     "M(Inr(a)) \<longleftrightarrow> M(a)"
by (simp add: Inr_def)

lemma (in M_trivial_no_repl_no_pow) Inr_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) M_converse_iff:
     "M(r) ==>
      converse(r) =
...
lemma (in M_basic_no_repl_no_pow) image_closed [intro,simp]:
     "[| M(A); M(r) |] ==> M(r``A)"
apply (simp add: image_iff_Collect)
apply (insert image_separation [of A r], simp)
done

lemma (in M_basic_no_repl_no_pow) vimage_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) relation_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) function_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) M_comp_iff:
     "[| M(r); M(s) |]
      ==> r O s =
...
lemma (in M_basic_no_repl_no_pow) restriction_abs [simp]:
     "[| M(f); M(A); M(z) |]
...
lemma (in M_basic_no_repl_no_pow) M_restrict_iff:
     "M(r) ==> restrict(r,A) = {z \<in> r . \<exists>x\<in>A. \<exists>y[M]. z = \<langle>x, y\<rangle>}"
by (simp add: restrict_def, blast dest: transM)

lemma (in M_basic_no_repl_no_pow) restrict_closed [intro,simp]:
     "[| M(A); M(r) |] ==> M(restrict(r,A))"
apply (simp add: M_restrict_iff)
apply (insert restrict_separation [of A], simp)
done

lemma (in M_basic_no_repl_no_pow) Inter_abs [simp]:
     "[| M(A); M(z) |] ==> big_inter(M,A,z) \<longleftrightarrow> z = \<Inter>(A)"
apply (simp add: big_inter_def Inter_def)
apply (blast intro!: equalityI dest: transM)
done

lemma (in M_basic_no_repl_no_pow) Inter_closed [intro,simp]:
     "M(A) ==> M(\<Inter>(A))"
by (insert Inter_separation, simp add: Inter_def)

lemma (in M_basic_no_repl_no_pow) Int_closed [intro,simp]:
     "[| M(A); M(B) |] ==> M(A \<inter> B)"
apply (subgoal_tac "M({A,B})")
apply (frule Inter_closed, force+)
done

lemma (in M_basic_no_repl_no_pow) Diff_closed [intro,simp]:
     "[|M(A); M(B)|] ==> M(A-B)"
by (insert Diff_separation, simp add: Diff_def)

subsubsection\<open>Some Facts About Separation Axioms\<close>

lemma (in M_basic_no_repl_no_pow) separation_conj:
...
lemma (in M_trivial_no_repl_no_pow) Collect_rall_eq:
...
lemma (in M_basic_no_repl_no_pow) separation_disj:
     "[|separation(M,P); separation(M,Q)|] ==> separation(M, \<lambda>z. P(z) | Q(z))"
by (simp del: separation_closed
         add: separation_iff Collect_Un_Collect_eq [symmetric])

lemma (in M_basic_no_repl_no_pow) separation_neg:
     "separation(M,P) ==> separation(M, \<lambda>z. ~P(z))"
by (simp del: separation_closed
         add: separation_iff Diff_Collect_eq [symmetric])

lemma (in M_basic_no_repl_no_pow) separation_imp:
...
lemma (in M_basic_no_repl_no_pow) separation_rall:
...
lemma (in M_basic_no_repl_no_pow) is_funspace_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) succ_fun_eq2:
     "[|M(B); M(n->B)|] ==>
      succ(n) -> B =
...
lemma (in M_trivial_no_repl_no_pow) Nil_in_M [intro,simp]: "M(Nil)"
by (simp add: Nil_def)

lemma (in M_trivial_no_repl_no_pow) Nil_abs [simp]: "M(Z) ==> is_Nil(M,Z) \<longleftrightarrow> (Z = Nil)"
by (simp add: is_Nil_def Nil_def)

lemma (in M_trivial_no_repl_no_pow) Cons_in_M_iff [iff]: "M(Cons(a,l)) \<longleftrightarrow> M(a) & M(l)"
...
lemma (in M_basic_no_repl_no_pow) list_case'_closed [intro,simp]:
...
lemma (in M_trivial_no_repl_no_pow) quasilist_abs [simp]:
     "M(z) ==> is_quasilist(M,z) \<longleftrightarrow> quasilist(z)"
by (auto simp add: is_quasilist_def quasilist_def)

lemma (in M_trivial_no_repl_no_pow) list_case_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) is_hd_Nil: "is_hd(M,[],Z) \<longleftrightarrow> empty(M,Z)"
by (simp add: is_hd_def)

lemma (in M_trivial_no_repl_no_pow) is_hd_Cons:
     "[|M(a); M(l)|] ==> is_hd(M,Cons(a,l),Z) \<longleftrightarrow> Z = a"
by (force simp add: is_hd_def)

lemma (in M_trivial_no_repl_no_pow) hd_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) is_tl_Nil: "is_tl(M,[],Z) \<longleftrightarrow> Z = []"
by (simp add: is_tl_def)

lemma (in M_trivial_no_repl_no_pow) is_tl_Cons:
     "[|M(a); M(l)|] ==> is_tl(M,Cons(a,l),Z) \<longleftrightarrow> Z = l"
by (force simp add: is_tl_def)

lemma (in M_trivial_no_repl_no_pow) tl_abs [simp]:
...
lemma (in M_trivial_no_repl_no_pow) relation1_tl: "relation1(M, is_tl(M), tl')"
by (simp add: relation1_def)
...
lemma (in M_basic_no_repl_no_pow) tl'_closed: "M(x) ==> M(tl'(x))"
apply (simp add: tl'_def)
apply (force simp add: quasilist_def)
done

(* lemma (in M_basic_no_repl_no_pow) cartprod_iff:
     "[| M(A); M(B); M(C) |]
...
locale M_basic_products = M_basic_no_repl_no_pow +
  assumes 
    fiber_replacement:
    "M(a) ==> strong_replacement(M, \<lambda>x y. y = <a,x>)"
    and
    product_replacement:
    "strong_replacement(M, \<lambda>x y. y = <a,x>)"
  

(*
lemma (in M_basic_no_repl_no_pow) cartprod_closed_lemma:
  assumes
    "M(A)"  "M(B)" 
  shows
    "\<exists>C[M]. cartprod(M,A,B,C)"
proof
*)  
...
lemma (in M_basic_no_repl_no_pow) cartprod_closed [intro,simp]:
  assumes
    "M(A)"  "M(B)" 
  shows
    "M(A*B)"
...
lemma (in M_basic_no_repl_no_pow) sum_closed [intro,simp]:
     "[| M(A); M(B) |] ==> M(A+B)"
by (simp add: sum_def)

lemma (in M_basic_no_repl_no_pow) sum_abs [simp]:
     "[| M(A); M(B); M(Z) |] ==> is_sum(M,A,B,Z) \<longleftrightarrow> (Z = A+B)"
by (simp add: is_sum_def sum_def singleton_0 nat_into_M)


lemma (in M_basic_no_repl_no_pow) converse_closed [intro,simp]:
     "M(r) ==> M(converse(r))"
apply (simp add: M_converse_iff)
apply (insert converse_separation [of r], simp)
done 

lemma (in M_basic_no_repl_no_pow) converse_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) vimage_closed [intro,simp]:
     "[| M(A); M(r) |] ==> M(r-``A)"
by (simp add: vimage_def) 

lemma (in M_basic_no_repl_no_pow) domain_closed [intro,simp]:
     "M(r) ==> M(domain(r))"
apply (simp add: domain_eq_vimage)
done 

lemma (in M_basic_no_repl_no_pow) field_abs [simp]:
     "[| M(r); M(z) |] ==> is_field(M,r,z) \<longleftrightarrow> z = field(r)"
by (simp add: is_field_def field_def) 

lemma (in M_basic_no_repl_no_pow) field_closed [intro,simp]:
     "M(r) ==> M(field(r))"
by (simp add: field_def) 

lemma (in M_basic_no_repl_no_pow) comp_closed [intro,simp]:
     "[| M(r); M(s) |] ==> M(r O s)"
apply (simp add: M_comp_iff)
apply (insert comp_separation [of r s], simp)
done

lemma (in M_basic_no_repl_no_pow) composition_abs [simp]:
...
lemma (in M_basic_no_repl_no_pow) restriction_is_function:
...
lemma (in M_basic_no_repl_no_pow) funspace_succ:
...
lemma (in M_basic_no_repl_no_pow) finite_funspace_closed [intro,simp]:
     "[|n\<in>nat; M(B)|] ==> M(n->B)"
