...
\begin{framed}
  ...
  \begin{bfseries}
    we assume that $\1\in\PP\in M$, $M$ is transitive and $1\in
    G\sbq\PP$.
  \end{bfseries}
...
\[
p\forces_{\PP,\leq,\1}^* \phi(\tau_1,\dots,\tau_n).
\]
Here, $\PP,\leq,\1,$ and $p\in\PP$ are the extra parameters. For the
time being, we just consider $\tau_1,\dots,\tau_n$ to be ordinary
variables. Informally, we call $\forces$ the \emph{forcing relation}.
Given $\quine\phi\in\formula$, we will write the forcing relation in
Isabelle/ZF by using  
\[
\forceisa(\quine\phi,\PP,\leq,\1)
\]


The base case (of atomic formulas) is famously the most
complicated one. 
