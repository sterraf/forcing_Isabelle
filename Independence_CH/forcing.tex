...
\end{itemize}
For example, the locale \locale{M{\uscore}Z{\uscore}basic}
...
In \session{ZF-Constructible} some results are proved assuming some instances of
separation or replacement (mejor si ponemos ejemplo concreto). In contrast,
our locale \isatt{M{\uscore}Z{\uscore}basic} assumes the full scheme of separation in terms
...
At this point, we work inside the locale \isatt{M{\uscore}ctm1} that
assumes $M$ to be
...
\end{isabelle}
%% where the $\isatt{map}$ function that applies a function to
