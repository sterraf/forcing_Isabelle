...
% If a concept is absolute, then one can reason ``externally'' instead
% of ``inside'' the model; thus, one has at hand any lemma about ``the''
% concept and not what the model believes of the concept.
...
(v.g. the Replacement Scheme).

In our version of the constructibility library,
...
get rid of tupling and thus various proofs are slicker.
...
For the case of the Unordered Pair Axiom, the statement for classes is the
following
\begin{isabelle}
