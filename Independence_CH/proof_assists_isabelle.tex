...
latter by means of \emph{tactics}; a %% forward rule changes the context
...
The metalogic of Isabelle, as well as LCF, is based on higher-order logic.
...
\footnote{%
We recommend the excellent survey \cite{2020arXiv200909541A} by
...
on which we work in this assistant. Both the kernel and the automation
...
quantifiers. The \isatt{arity} function on $\phi$ is one plus the
...
%% \isatt{arity}(\phi)$.\textbf{$\leftarrow$ necessary?}}
This completes the
