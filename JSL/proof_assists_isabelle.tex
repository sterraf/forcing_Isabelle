...
the object logic appears as atomic for $\Meta$.
...
This is an example of what was said at the end of the previous
section.
...
Isabelle/ZF treatment of inductively defined datatypes; induction
and recursion theorems for them are proved automatically.

To avoid problems related to the binding of variables, de Bruijn
indices are used instead. The satisfaction predicate
$\isatt{sats}::\tyi\fun\tyi\fun\tyi\fun\tyo$ takes as arguments a set $M$, a list
$\mathit{ms}\in\isatt{list}(M)$ for the assignment of free indices,
and $\phi\in\formula$, and it is written
$M,\mathit{ms}\models\phi$ in our formalization. This completes the
...
concepts is obtained by guiding the automatic tactics. Actually,
great many of the concepts in \session{ZF-Constructible} where
internalized by hand, as is the case for union,
%% \begin{multline*}
...
\begin{multline*}
  ...
  ) \longleftrightarrow  \isatt{big{\uscore}union}(\isatt{\#\#} A, \isatt{nth}(x,
