...
working set-theorist. \fbox{un poquito jugado lo anterior, lo vemos} In the current paper we reported a first major
...
for the existence of generic filters.  \fbox{Se me hace que la
  anécdota empieza con forces} \fbox{que agrega cuatro variables} A
more anecdotal one is that to show that any instance of Separation
with at most two parameters holds in $M[G]$, one needs to assume every
instance with six parameters in $M$ \fbox{Chequear los usos de any y
  every}. The purpose of identifying those assumptions is to assemble
...
somewhat differently: a trivial change is to catalogue in one file all
the internalized formulas \fbox{No tiene la desventaja de que puede
  llegar a ser necesario} \fbox{ agregar más en un futuro.}. A more
...
many absoluteness results follow from this hypothesis \fbox{Estaba en
  plural hipótesis, por qué?}. Furthermore, as Paulson comments in the
...
satisfy a sub-theory of $\ZF$ not including Powersete, and to ensure a
broader applicability, it would be convenient to have absoluteness
results not assuming it. Once we gained more confidence that these
ideas, we plan to contribute back to the official distribution of
Isabelle/ZF with a thorough revision of the development of
constructibility.

\section*{Acknowledgment}

The second author is grateful to the others two. They are very nice persons.
