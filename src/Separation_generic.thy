      ...
      proof (intro ballI)
        ...
        with Eq1 and Eq2 and Eq3 have
                "sats(M,?\<psi>,[u] @ ?env @ [\<pi>,\<sigma>]) \<longleftrightarrow> 
                  (\<exists>\<theta>\<in>M. \<exists>p\<in>M. u =<\<theta>,p> \<and> sats(M,forces(?\<chi>),[\<theta>,p,u]@?env@[\<pi>,\<sigma>]))"
          by (auto simp add: transitivity)
        also have
                " ... \<longleftrightarrow> 
                  (\<exists>\<theta>\<in>M. \<exists>p\<in>P. u =<\<theta>,p> \<and> sats(M,forces(?\<chi>),[\<theta>,p,u]@?env@[\<pi>,\<sigma>]))"
          using Eq4 P_in_M by (auto simp add:transitivity)
        also have
