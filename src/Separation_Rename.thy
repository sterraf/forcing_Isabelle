...
proof -
  {
    ...
    then 
    have "x\<in>(m#+n)-m" using ltD DiffI N by simp
  }
  then have A: "weak(n,m)\<subseteq>(m#+n)-m" using subsetI by simp
  {
    fix x 
    assume "x\<in>(m#+n)-m"
    then 
  ...
  then have "(m#+n)-m\<subseteq>weak(n,m)" using subsetI by simp
...
proof(auto)
  ...
  then 
  show "\<And>xa. n \<in> nat \<Longrightarrow> succ(succ(natify(xa))) \<noteq> n \<Longrightarrow> \<forall>x\<in>n. succ(succ(natify(xa))) \<noteq> natify(x) \<Longrightarrow> xa \<in> n \<Longrightarrow> succ(natify(xa)) = n"
    by blast
qed
...
proof - 
  ...
  {fix x
    ...
    with \<open>x=5#+i\<close> have "x < 5#+n" by simp
  }
  then 
  have R: "x < 5#+n" if "x\<in>?A" for x using that by simp
  then have 1:"x\<notin>?A" if "\<not>x <5#+n"  for x using that by blast
  ...
  qed
  then have E:"x\<notin>?A" if "x\<in>domain(sep_var(n))" for x 
    unfolding weak_def
    using C that by force
  then have F: "domain(sep_var(n)) \<subseteq> 7#+n - ?A" using A by auto
  from assms
  ...
  moreover
  {
    ...
    proof (cases "2 \<le> n")
      ...
      have "x<5"
        ...
        apply(subgoal_tac "0\<in>n", auto, rule succ_In, simp_all)
        done
      then show ?thesis unfolding sep_var_def using \<open>n\<in>nat\<close> 
    ...
    next
      ...
      next
        ...
        then have "n=1" using not_lt_iff_le \<open>n<2\<close> le_iff by auto
        then show ?thesis 
...
lemma sep_var_action :
  ...
  apply (subst apply_fun[OF sep_var_type],simp add: length_type[OF \<open>env\<in>list(M)\<close>],simp add: sep_var_def,simp)
  apply ( simp add: nth_concat3[OF \<open>env\<in>list(M)\<close>,symmetric])
  done
...
definition sep_ren :: "[i,i] \<Rightarrow> i" where
  "sep_ren(n,\<phi>) == ren(\<phi>)`(7#+n)`(7#+n)`rensep(n)"
