...
proof (e.g.\ of a contradiction)\footnote{We purportedly mention this
  example because results about the independence of some axiom $A$
  with respect to $\ZFC$ can be stated as transforming a proof
...
task of transferring truth of axioms in $M$ to $L^M$, and consitute
...
countable transitive models; the extension of a ctm $M$ by the filter
$G$ is denoted by $M[G]$. In general, no such $G$ belongs to $M$ and
...
% The elements of $M[G]$ are the decodification of certain elements of
% $M$, called \emph{names}. The decoding is given by the function
% $\isa{val}$ which takes the generic filter $G$ as a parameter. To
% prove that $M$ is contained in $M[G]$ it suffices to give a name for
% each element of $M$; we define the function \isa{check} which assigns
% a name for each $x\in M$. Showing that \isa{check(x)\ \isasymin\ M}
% when \isa{x\ \isasymin\ M} involves some technical issues that will
% be formalized in a further work. We explain names, \isa{val}, and
% \isa{check} in Section~\ref{sec:names}.

Elements of $M[G]$ are coded by elements of $M$, and are obtained from
them by applying a function \isa{val} that uses the generic filter as
a parameter. In order to show that $M[G]$ does extend $M$, we
formalize the function \isa{check} that given an element $x\in M$
provides with a code (a ``name'') for it. Show that this name belongs
to $M$ involves some technical issues that will be addressed in a
further work. This is all explained in Section~\ref{sec:names}.
